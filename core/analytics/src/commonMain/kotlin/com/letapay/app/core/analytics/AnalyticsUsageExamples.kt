...
class LetaPayAnalyticsExample(private val analytics: AnalyticsHelper) {

    private val letapayTracker = analytics.letapayTracker()

    fun demonstrateLetaPayFeatures() {
        // Client operations
        letapayTracker.trackClientOperation(
            operation = "create",
            clientId = "C12345",
            success = true,
            duration = 1500L,
        ...
        // Loan operations
        letapayTracker.trackLoanOperation(
            operation = "apply",
            loanType = "personal",
            amount = "50000",
            success = true,
        ...
        // Savings operations
        letapayTracker.trackSavingsOperation(
            operation = "deposit",
            accountId = "S67890",
            amount = "10000",
            success = true,
        ...
        letapayTracker.trackPerformance(
...
fun ClientDetailsScreen(
    ...
    val documentTracker = rememberLetaPayDocumentTracker()
    val letapayAnalytics = rememberLetaPayAnalyticsTracker()
    ...
    Column(
        ...
        Button(
            ...
            onClick = {
                analytics.logEvent(
                    "client_action",
                    LetaPayParamKeys.CLIENT_ID to clientId,
        ...
        Button(
            onClick = {
                letapayAnalytics.trackClientOperation("update", clientId, success = true)
                onNavigateBack()
            },
        ) {
            Text("Save Changes")
...
fun SurveyScreen(surveyId: String) {
    val analytics = rememberLetaPayAnalyticsTracker()
    var currentQuestion by remember { mutableStateOf(1) }
    val totalQuestions = 5

    // Track survey start
    TrackLetaPaySurvey(surveyId, action = "started")
...
@Composable
fun ReportScreen() {
    val reportTracker = rememberLetaPayReportTracker()
    val analytics = rememberAnalyticsHelper()
...
object AnalyticsDIExample {
    ...
    fun setupAnalyticsDI() = module {
        ...
        // Mifos tracker
        single<LetaPayAnalyticsTracker> { LetaPayAnalyticsTracker(get()) }
