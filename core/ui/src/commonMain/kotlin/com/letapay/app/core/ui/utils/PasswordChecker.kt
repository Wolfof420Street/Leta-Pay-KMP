 ...
 */
package com.letapay.app.core.ui.utils
...
object PasswordChecker {
    ...
    fun getPasswordStrengthResult(password: String): PasswordStrengthResult {
        when {
            password.isEmpty() -> return PasswordStrengthResult.Error(PasswordError.EMPTY)
            password.length > MAX_PASSWORD_LENGTH -> {
                return PasswordStrengthResult.Error(PasswordError.TOO_LONG)
            }
        }
    ...
    fun getPasswordFeedback(password: String): List<PasswordFeedback> {
        val feedback = mutableListOf<PasswordFeedback>()

        if (password.length < MIN_PASSWORD_LENGTH) {
            feedback.add(PasswordFeedback.MINIMUM_LENGTH)
        }
        if (!password.any { it.isUpperCase() }) {
            feedback.add(PasswordFeedback.UPPERCASE)
        }
        if (!password.any { it.isLowerCase() }) {
            feedback.add(PasswordFeedback.LOWERCASE)
        }
        if (!password.any { it.isDigit() }) {
            feedback.add(PasswordFeedback.NUMBER)
        }
        if (!password.any { !it.isLetterOrDigit() }) {
            feedback.add(PasswordFeedback.SPECIAL_CHARACTER)
        }
        if (password.length < STRONG_PASSWORD_LENGTH) {
            feedback.add(PasswordFeedback.STRONGER_LENGTH)
        }
...
sealed class PasswordStrengthResult {
    ...
    data class Error(val error: PasswordError) : PasswordStrengthResult()
}
