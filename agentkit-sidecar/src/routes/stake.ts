...
const StakeSchema = z.object({
  ...
  opportunityId: z.enum(["LIDO_STETH_STAKE", "AAVE_V3_SUPPLY_WETH"]),
  amount: z.string(),
...
router.post("/build", async (req, res, next) => {
  try {
    ...
    const isLido = body.opportunityId === "LIDO_STETH_STAKE";
    const to = isLido
      ? "0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84"
      : "0x87870Bca3F3fD6335C3f4ce8392D69350B4fA4E2";
    const data = isLido
      ? "0xa1903eab"
      : `0x617ba037${padAddress("0xC02aaA39b223FE8D0A0E5C4F27eAD9083C756Cc2")}${padUint(amountWei)}${padAddress(body.fromAddress)}${padUint(0n)}`;
    const value = isLido ? `0x${amountWei.toString(16)}` : "0x0";
